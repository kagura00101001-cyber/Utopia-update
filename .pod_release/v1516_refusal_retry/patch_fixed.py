...
# Scope the catch insertion to processBatch only; the file contains other catches with the same opening lines.
process_start=user.index('  async function processBatch(){')
process_end=user.index('  async function clearComposer(){',process_start)
catch_marker="      }catch(e){\n        if(e instanceof PausedError)throw e;"
catch_pos=user.index(catch_marker,process_start,process_end)
insert_at=catch_pos+len(catch_marker)
refusal_branch=r'''
...
user=user[:insert_at]+refusal_branch+user[insert_at:]

# All explicit task reset entry points clear the persisted refusal retry counter.
...
print('POD V1.5.16 fixed patch applied')
